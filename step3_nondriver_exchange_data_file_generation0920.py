...
def generate_var_files():
    ...
    for case_id in case_ids:
        ...
        if CASE_ID_LIST and case_id not in CASE_ID_LIST:
            ...
            continue
        # ----------------------------------------------
        None_driver_case_id = case_id + NON_DRIVER_OFFSET
        print(f"正在处理主驾工况: {case_id}...，对应副驾id为 {None_driver_case_id}")
        # --- 3.1. 获取当前工况的参数值 ---
        ...
        occupant_type = data.loc[None_driver_case_id, 'occupant_type']
        ll1_kn = data.loc[None_driver_case_id, 'll1']
        ll2_kn = data.loc[None_driver_case_id, 'll2']
        btf_ms = data.loc[None_driver_case_id, 'btf']
        pp_mm = data.loc[None_driver_case_id, 'pp']
        ptf_ms = data.loc[None_driver_case_id, 'ptf']
        plp_mm = data.loc[None_driver_case_id, 'plp']
        lla_status = data.loc[None_driver_case_id, 'lla_status']
        llattf_ms = data.loc[None_driver_case_id, 'llattf']
        dz_level = data.loc[None_driver_case_id, 'dz']
        aft_ms = data.loc[None_driver_case_id, 'aft']
        aav_status = data.loc[None_driver_case_id, 'aav_status']
        ttf_ms = data.loc[None_driver_case_id, 'ttf']
        sp_mm = data.loc[None_driver_case_id, 'sp']
        recline_deg = data.loc[None_driver_case_id, 'recline_angle']
        ...
        if (pd.isnull(param_values).any() or 
            ...
            print(f"  - !!警告：工况ID {None_driver_case_id} 存在缺失参数，跳过该工况。")
            continue
        ...
        for axis, var_name in pulse_params.items():
            pulse_filename = f'{axis}{case_id}.csv'  # 注意这里使用主驾case_id读取文件，因为主副驾共享同一碰撞波形文件
        ...
        # --- 3.5. 写入最终的.var文件 ---
        if not bad_flag:
            output_filename = f'data_{None_driver_case_id}.var'
