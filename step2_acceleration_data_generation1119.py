...
g=9.81    #单位m/s2
xls_route = r'I:\000 LX\dataset0715\02\results_all_before1111'
output_path = r'I:\000 LX\dataset0715\03\acc_data_all_before1111_new'
params_path = r'I:\000 LX\dataset0715\02\distribution_1111.csv' 
...
for file in path_list:
    ...
    # if file_size_kb < 14800: # 150ms 0.005ms则为14000KB
    ...
    # 只读取需要的列，减少内存占用和读取时间
    usecols = [0, 17, 19, 37]  # 只读取需要的列：时间、vx、vy、omg_z
    ...
    vx, vy = data_values[:, 1], data_values[:, 2]  # m/s
    omg_z = data_values[:, 3]  # rad/s
    ...
    #  # ****** Z方向 角加速度 ******
    omg_z_filtered = CFC_filter(tim, omg_z, 60)
    eps_z = np.gradient(omg_z_filtered, tim)  # rad/s^2
    eps_z_filtered = CFC_filter(tim, eps_z, 60)
    z_name = 'z' + str(case_id) + '.csv' 
    save_csv({'XI': tim, 'YI': eps_z_filtered}, z_name, output_path)
