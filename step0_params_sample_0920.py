...
def sample_collision_params(n_samples=6000, skip_points=1024, method='uniform', 
    ...
    if method == 'uniform':
        ...
        # 特殊处理重叠率: (-1, -0.15]∪[0.15, 1]
        # 将[0,1)映射到两个区间的联合：(-1, -0.15]∪[0.15, 1]
        u = uniform_samples[:, 2]  # [0, 1)范围的均匀样本

        # 计算两个区间的长度
        interval1_length = -0.15 - (-1.0)  # 0.85
        interval2_length = 1.0 - 0.15      # 0.85
        total_length = interval1_length + interval2_length  # 1.7

        # 将[0,1)按比例分配到两个区间
        threshold = interval1_length / total_length  # ≈0.5

        # 对于u < threshold的样本，映射到(-1, -0.15]
        # 对于u >= threshold的样本，映射到[0.15, 1]
        overlap = np.where(
            u < threshold,
            -1.0 + (u / threshold) * interval1_length,  # 映射到(-1, -0.15]
            0.15 + ((u - threshold) / (1 - threshold)) * interval2_length  # 映射到[0.15, 1]
        )
...
sample_collision_params(n_samples=7000, skip_points=5048, method='non_uniform', filename='distribution_test.csv', seed=20252025, case_ids=np.arange(3001, 10001))
