...
if __name__ == "__main__":
    ...
    # 示例1：您提出的问题工况 (speed=25, overlap=25)
    print("测试工况: 速度=25 kph, 重叠率=25% (表格值为OFF=100)")
    print("现在应该有随机性了:")
    # 邻居点是 (35,25)->50 和 (25,50)->45。
    # 范围由 [100, 50, 45] 决定，即 [45, 100]。Mode是100。
    for i in range(5):
        btf_sample = sampler.sample(25, 25)
        print(f"  采样 {i+1}: {btf_sample:.2f} ms")
    print("-" * 30)

    # 示例2：测试一个表格中间的点 (speed=45, overlap=50)
    print("测试工况: 速度=45 kph, 重叠率=50% (表格值为25)")
    print("根据邻居节点进行随机采样:")
    # 邻居点: (35,50)->35, (55,50)->15, (45,25)->45, (45,75)->25
    # 范围由 [25, 35, 15, 45, 25] 决定，即 [15, 45]。Mode是25。
    for i in range(5):
        btf_sample = sampler.sample(45, 50)
        print(f"  采样 {i+1}: {btf_sample:.2f} ms")
    print("-" * 30)
    ...
    speed3 = 40
    overlap3 = -60
    print(f"测试工况: 速度={speed3} kph, 重叠率={overlap3}% (插值工况)")
    print("进行5次采样:")
    for i in range(5):
